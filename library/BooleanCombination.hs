...
bcImplies primImplies x y = case (x,y) of
  ...
  (Or as, b) | all (`implies` b) as -> True
  (a, Or bs) | any (\b -> a `implies` b) bs -> True
  (And as, b) | any (`implies` b) as -> True
  (a, And bs) | all (\b -> a `implies` b) bs -> True
  _ -> False
  where
